...
const Profile = () => {
  ...
  useEffect(() => {
    fetch(`https://api.github.com/users/${username}`)
      ...
      .then((res) => {
        console.log("resFirst", res), setData(res);
      })
      .catch((err) => console.log("Error fecthing api:", err));
      ...
      .then((res) => {
        console.log("resSecond", res), setRepos(res);
      })
      .catch((err) => console.log("Error fecthing api:", err));
  }, []);
                ...
                <div className="badges my-1 flex w-full justify-evenly flex-wrap">
                  ...
                  <div className="badge badge-secondary cursor-pointer ">
                    <a href={`https://github.com/${username}?tab=followers`}>
                      <b>Seguidores: {data.followers}</b>
                    </a>
                  </div>
                  <div className="badge badge-accent cursor-pointer">
                    <a href={`https://github.com/${username}?tab=following`}>
                      <b>Seguindo: {data.following}</b>
                    </a>
                  </div>
                </div>
                <div className="mt-3 flex items-center">
                  {data.email && <GoLocation />}

                  <p className="ml-2 text-xl">{data.email}</p>
                </div>

                <div className="mt-3 flex items-center">
                  <p className="ml-2 text-xl">{data.bio}</p>
                </div>
